...
// CIE 1931 2-degree color matching functions tabulated at 5nm intervals, 380-780nm.
// Source: CIE publication 15:2004
namespace CIE {
    constexpr double LAMBDA_MIN  = 380.0;
    constexpr double LAMBDA_MAX  = 780.0;
    constexpr double LAMBDA_STEP = 5.0;
    constexpr int    N_SAMPLES   = 81;

    // x-bar
    constexpr std::array<double, N_SAMPLES> X_BAR = {
    ...
    // y-bar
    constexpr std::array<double, N_SAMPLES> Y_BAR = {
    ...
    // z-bar
    constexpr std::array<double, N_SAMPLES> Z_BAR = {
    ...
    // Sample all three CMFs at wavelength lambda (nm) via linear interpolation
    inline std::tuple<double, double, double> sample(double lambda) {
        if (lambda < LAMBDA_MIN || lambda > LAMBDA_MAX)
            return {0.0, 0.0, 0.0};

        double t   = (lambda - LAMBDA_MIN) / LAMBDA_STEP;
        int    idx = static_cast<int>(t);
        double frac = t - idx;

        if (idx >= N_SAMPLES - 1)
            return {X_BAR[N_SAMPLES-1], Y_BAR[N_SAMPLES-1], Z_BAR[N_SAMPLES-1]};

        return {
            X_BAR[idx] + frac * (X_BAR[idx+1] - X_BAR[idx]),
            Y_BAR[idx] + frac * (Y_BAR[idx+1] - Y_BAR[idx]),
            Z_BAR[idx] + frac * (Z_BAR[idx+1] - Z_BAR[idx])
        };
    }

    // Normalization constant: integral of y-bar over visible spectrum
    // Used to normalize XYZ so that a perfect white reflector gives Y=1
    constexpr double Y_INTEGRAL = 106.856895;
}
